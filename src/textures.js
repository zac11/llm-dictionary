...
export function makeCoverTexture({ label, letters, count, c1, c2, roman }) {
  ...
  ctx.fillText('✦  N E U R O P A E D I A  ✦', W / 2, 96);
...
export function makeSpineTexture({ label, letters, count, c1, c2, roman }) {
  ...
  // title, rotated like a real spine (sized so the longer wordmark clears
  // the big range letters above and the roman/term count below)
  ctx.save();
